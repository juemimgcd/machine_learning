...
# 使用决策树CART、贝叶斯分类、KNN、逻辑回归、随机森林RF、bagging、GBDT等算法，分别对iris数据、breast_cancer数据进行分析：
#
# （1）需要分别使用test_split方法和K-fold交叉检验的方法；
#
# （2）详细分析这几类算法的预测准确性，并选取cart算法为基准算法进行比较，在同一个图中绘制不同算法性能指标曲线；
#
# （3）对所得到的分析结果进行可解释性说明。
...
cart_f_score = f1_score(y_test,y1_pre,average='weighted')
...
nb_f_score = f1_score(y_test,y2_pre,average='weighted')
...
model3 = LogisticRegression(solver='sag', random_state=42, max_iter=5000, tol=1e-4,multi_class='auto')
model3.fit(x_scaled, y_train)
x_test_scaled = scaler.transform(x_test)
y3_pre = model3.predict(x_test_scaled)
lg_f_score = f1_score(y_test,y3_pre,average='weighted')
...
rf_f_score = f1_score(y_test,y_pre4,average='weighted')
...
bg_f_score = f1_score(y_test,y_pre5,average='weighted')
...
gb_f_score = f1_score(y_test,y_pre6,average='weighted')
...
kn_f_score = f1_score(y_test,y_pre7,average='weighted')
